...
impl LocationMutatorActor{
    ...
    pub async fn raw_store(&mut self, message: LocationEventMessage){
        ...
        let nv = NaiveDateTime::parse_from_str(&naive_dt, "%Y-%m-%dT%H:%M:%SZ").unwrap();
        let fixed_offset = FixedOffset::east_opt(0).unwrap();
        let dt_fixed_offset = DateTime::<FixedOffset>::from_naive_utc_and_offset(nv, fixed_offset);
        ...
        match db.execute(insert_stmt).await{
            ...
            Err(e) => {
                use crate::error::{ErrorKind, AcumerErrorResponse};
