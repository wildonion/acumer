...
// more tcp logics in: https://github.com/wildonion/acumer/blob/main/src/helpers/tcpserver.rs
#[post("/test-stream")]
pub(self) async fn test_stream(
...
) -> Result<actix_web::HttpResponse, crate::error::AcumerErrorResponse>{
    ...
    // AcumerErrorResponse in respond to the client since the type of Error 
    // part of open_file() method is the custom FileError type, we can't 
    // return it in this method which it's error part is of type AcumerErrorResponse
    // we should build the AcumerErrorResponse manually from FileError type
    // ...
    ...
    // the error into AcumerErrorResponse not the its KindaError enum variant, we use
    ...
    // since we're handling the error using AcumerErrorResponse there is no need to match over
    // ok or the err part of the result we can directly use ? operator Rust will take care of 
    // the rest process.
    // we can use ? operator since the From<std::io::Error> trait has implemented for the AcumerErrorResponse
