...
if [[ $ENVCOMPLETED == "Y" || $ENVCOMPLETED == "y" ]]; then
    ...
    if [[ $REDPLOY_INFRASTRUCTURE == "Y" || $REDPLOY_INFRASTRUCTURE == "y" ]]; then
        ...
        docker run -d --restart unless-stopped --name dbeaver --network acumer -ti -p 8080:8978 -v $(pwd)/infra/data/opt/cloudbeaver/workspace dbeaver/cloudbeaver:latest
        ...
        sudo docker run -d --network acumer -p 7050:3000 --name=grafana --user "$(id -u)" --volume $(pwd)/infra/data:/var/lib/grafana grafana/grafana # admin | admin
        ...
        sudo docker run -d --network acumer --name timescaledb -p 5432:5432 \
        ...
        sudo docker run -d --network acumer --hostname rabbitmq -p 5672:5672 -p 15672:15672 --name rabbitmq -e RABBITMQ_DEFAULT_USER=acumer -e RABBITMQ_DEFAULT_PASS=$PASSWORD rabbitmq:3-management

        sudo docker run -d --link timescaledb --network acumer --name adminer -p 7543:8080 adminer
        sqlx database create
        sqlant postgresql://postgres:$PASSWORD@localhost/acumer > $(pwd)/infra/acumer.uml
        java -jar $(pwd)/infra/plantuml.jar $(pwd)/infra/acumer.uml
        ...
        ANY_RUSTACKI_HTTP_CONTAINER_ID=$(docker container ls  | grep 'acumer-http' | awk '{print $1}')
        sudo docker stop $ANY_RUSTACKI_HTTP_CONTAINER_ID && sudo docker rm -f $ANY_RUSTACKI_HTTP_CONTAINER_ID

        ANY_RUSTACKI_GRPC_CONTAINER_ID=$(docker container ls  | grep 'acumer-grpc' | awk '{print $1}')
        sudo docker stop $ANY_RUSTACKI_GRPC_CONTAINER_ID && sudo docker rm -f $ANY_RUSTACKI_GRPC_CONTAINER_ID

        ANY_RUSTACKI_TCP_CONTAINER_ID=$(docker container ls  | grep 'acumer-tcp' | awk '{print $1}')
        ...
        sudo docker build -t acumer-http-$TIMESTAMP -f $(pwd)/infra/docker/acumer/Dockerfile . --no-cache
        sudo docker run -d --restart unless-stopped --link timescaledb --network acumer --name acumer-http-$TIMESTAMP -p 2346:2344 -v $(pwd)/assets/:/app/assets -v $(pwd)/infra/logs/:/app/logs acumer-http-$TIMESTAMP

        sudo docker build -t acumer-grpc-$TIMESTAMP -f $(pwd)/infra/docker/acumer/Dockerfile . --no-cache
        sudo docker run -d --restart unless-stopped --link timescaledb --network acumer --name acumer-grpc-$TIMESTAMP -p 2257:2256 -v $(pwd)/assets/:/app/assets -v $(pwd)/infra/logs/:/app/logs acumer-grpc-$TIMESTAMP

        sudo docker build -t acumer-tcp-$TIMESTAMP -f $(pwd)/infra/docker/acumer/Dockerfile . --no-cache
        sudo docker run -d --restart unless-stopped --link timescaledb --network acumer --name acumer-tcp-$TIMESTAMP -p 2735:2734 -v $(pwd)/assets/:/app/assets -v $(pwd)/infra/logs/:/app/logs acumer-tcp-$TIMESTAMP
